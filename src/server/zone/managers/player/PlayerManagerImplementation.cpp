...
void PlayerManagerImplementation::loadLuaConfig() {
	...
	baseStoredCreaturePets = lua->getGlobalInt("baseStoredFamiliarPets");
	...
	*trickTreatScreenplay << player;
	...
	if (attacker->isPlayerCreature()) {
		...
		if (!CombatManager::instance()->areInDuel(attackerCreature, player)) {
			...
			box->setPromptText("Place a bounty on your killer! Enter an amount between 25k and 500k credits. The Bounty Hunters Guild will take 20% for their fees and your target will be added to our boards immediately.");
