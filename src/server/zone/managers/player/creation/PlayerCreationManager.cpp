...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
					...
					try {
						...
						if (res != NULL && res->next()) {
							...
							if (timeVal.miliDifference() < 43200000) {
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per 12 hours. Repeat attempts prior to 12 hour elapsing will reset the timer.", 0x0);
								client->sendMessage(errMsg);
	...
	box->setPromptText("You are limited to creating one character per 12 hour period. Attempting to create another character or deleting your character before the 12 hour timer expires will reset the timer.");

	ghost->addSuiBox(box);
	String playerName = playerCreature->getFirstName();
	StringBuffer zBroadcast;
	zBroadcast << "\\#FF2400 Player " << playerName << " has entered the galaxy, welcome!";
