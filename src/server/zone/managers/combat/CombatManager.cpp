...
int CombatManager::getHitChance(TangibleObject* attacker, CreatureObject* targetCreature, WeaponObject* weapon, int damage, int accuracyBonus) {
	...
	if (damage > 0) {
		...
		if (def == "saber_block") {
			if (attacker->isPlayerCreature()) {
				if (!(attacker->isTurret() || weapon->isThrownWeapon()) && ((weapon->isHeavyWeapon() || weapon->isSpecialHeavyWeapon() || (weapon->getAttackType() == SharedWeaponObjectTemplate::RANGEDATTACK))) && ((System::random(100 + (accTotal / 7))) < targetCreature->getSkillMod(def)))
					return RICOCHET;
				else return HIT;
			} else {
