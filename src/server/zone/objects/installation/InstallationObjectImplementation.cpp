...
void InstallationObjectImplementation::setOperating(bool value, bool notifyClient) {
	...
	if (operating) {
		...
		uint64 ownerid = getOwnerObjectID();
		ZoneServer* zoneServer = getZoneServer();
		ManagedReference<SceneObject*> ownerObject = zoneServer->getObject(ownerid);
		if (ownerObject->isCreatureObject()){
			CreatureObject* player = ownerObject.castTo<CreatureObject*>();
			ChatManager* chatManager = zoneServer->getChatManager();
			String planet = getZone()->getZoneName();
			StringBuffer msg;
			msg << "Your harvester at " << String::valueOf(getPositionX()) << ", " << String::valueOf(getPositionY()) << " on " << planet << " has stopped working." << endl;
				
			chatManager->sendMail("System", "Harvester Stopped", msg.toString(), player->getFirstName());
		}
	}
