...
void run(CreatureObject* creature, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		try
		{
			...
			missionManager->addPlayerToBountyList(player->getObjectID(), newBounty);
			...
			creature->addBankCredits(refund);
				...
				zBroadcast << "\\#ffffff The Bounty Hunters guild has acquired a new target.\\#ffffff";
